...
# Schoenfeld residuals intuition plot

RealH0 <- 0.01+0.01*T
PropH1 <- 0.02+0.02*T
RealH1 <- rep(0.35, length=length(T))

pdf("SchoenIntuition.pdf",6,5)
par(mar=c(4,4,2,2))
plot(T,RealH0,t="l",ylim=c(0,0.65),lty=1,
     lwd=3,col="black",ylab="Hazard")
lines(T,RealH1,lwd=3,lty=2,col="black")
lines(T,PropH1,lwd=3,lty=2,col="red")
text(20,0.16,labels="h(t|X=0)")
text(7,0.375,labels="Actual h(t|X=1)")
text(20,0.6,labels="Fitted (proportional) h(t|X=1)",
     col="red")
arrows(2,0.34,2,0.07,code=3,lwd=1.5,length=0.1,col="darkgreen")
arrows(29,0.36,29,0.59,code=3,lwd=1.5,length=0.1,col="darkgreen")
text(5.1,0.22,labels="Underprediction",cex=0.75,col="darkgreen")
text(26.1,0.46,labels="Overprediction",cex=0.75,col="darkgreen")
dev.off()
